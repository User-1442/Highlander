...
void main() {
    ...
    for (int i = 0; i < numLights; i++) {
        ...
        float shadowconverteddist = dist / 100.0f;

        if (texture(shadowmap, FragmentWorld - LightPos[i].xyz).r < shadowconverteddist - bias) {
            visibility = 0.5;
        }
	...
	gAlbedo = vec4((ambient + totalDiffuse + finalSpecular) * visibility, 1);
