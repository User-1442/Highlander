...
void main() {
    ...
    vec3 R = normalize(mix(reflect(I, N), vec3(hash(TexCoords + 0.1) * 2.0 - 1.0, hash(TexCoords + 0.2) * 2.0 - 1.0, hash(TexCoords + 0.3) * 2.0 - 1.0), roughness * 0.5));
    ...
    float inversend = 1.0 / (projection * vec4(viewPos.xyz + (R * 10), 1.0)).w;
    ...
    for (int i = 0; i < 30; i++) {
        ...
        if (raypos.z > newdepth && raypos.z < newdepth + 0.02 && newdepth != 1.0 && mask != 0.0) {
            ...
            FragColor = vec4(mix(reflections, vec3(sceneColor), mask * (reflections.r + reflections.g + reflections.b) / 3), 1.0);
            break;
        }
    }
}
