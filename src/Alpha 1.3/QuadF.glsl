...
void main() {
    ...
    vec3 R = normalize(reflect(I, N));// WHEN REPAIRING ADD mix() after normalize, vec3(hash(TexCoords + 0.1) * 2.0 - 1.0, hash(TexCoords + 0.2) * 2.0 - 1.0, hash(TexCoords + 0.3) * 2.0 - 1.0), 0.05f));//, vec3(hash(TexCoords + 0.1) * 2.0 - 1.0, hash(TexCoords + 0.2) * 2.0 - 1.0, hash(TexCoords + 0.3) * 2.0 - 1.0), 0.0f)); //ALSO WHEN REPAIRING IT PUT THE MIX BY ROUGHNESS BACK
